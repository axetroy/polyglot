...
function makePng() {
  ...
  const png = new Uint8Array(sig.length + chunk('IHDR', ihdr).length + chunk('IDAT', idat).length + chunk('IEND', new Uint8Array(0)).length);
  let off = 0;
  png.set(sig, off); off += sig.length;
  png.set(chunk('IHDR', ihdr), off); off += chunk('IHDR', ihdr).length;
  png.set(chunk('IDAT', idat), off); off += chunk('IDAT', idat).length;
