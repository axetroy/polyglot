...
describe("Engine error paths", () => {
  ...
  it("create() throws IncompatibleFormatError for unsupported but registered pair", async () => {
    ...
    engine.registry.registerBack({
      ...
      async parse() {
        return { entries: [], centralDir: [], eocd: {} as unknown, raw: Buffer.from([]) };
      },
      getLayout() {
        return Promise.resolve({ format: "foo" as const, size: 0 });
      },
      relocate(buf: Buffer) {
        return buf;
      },
      toSource() {
        return { size: () => Promise.resolve(0), read: () => Promise.resolve(Buffer.from([])) };
      },
      toPathSource() {
        return { size: () => Promise.resolve(0), read: () => Promise.resolve(Buffer.from([])) };
      },
  ...
  it("openFront() throws on non-polyglot file", async () => {
    ...
    await expect(engine.openFront(join(TEST_DIR, "plain.png"))).rejects.toThrow(InvalidFrontError);
  });
  ...
  it("openBack() throws on non-polyglot file", async () => {
    ...
    await expect(engine.openBack(join(TEST_DIR, "plain.png"))).rejects.toThrow(InvalidArchiveError);
  });
  ...
  it("archive.read() throws when entry not found", async () => {
    ...
    await expect(archive.read("nonexistent.txt")).rejects.toThrow(/not found/);
  });
});
...
describe("JPEG validate edge cases", () => {
  it("validate() returns false when EOI is missing", async () => {
    ...
    const buf = Buffer.from([
      ...
      0xff, 0xda, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3f, 0x00, 0x00, 0x00,
      // NO 0xFFD9
    ]);
